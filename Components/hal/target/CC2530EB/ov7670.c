...
uint8 OV7670_Init(void)
{
    ...
    P1DIR &= 0x00;
	P1INP &= 0x00;
    P2INP |= 0xBF; // p1 port all pull-up mode
	
    // init control
    // p0_0,1,4,5 6 output/general I/O/pull-up Mode
    P0SEL &= 0x8C;
    P0DIR |= 0x73;
    P0INP |= 0x83;
	P2INP &= 0xDF;
    ...
    // init Vsync(p2_0)
    P2IEN |= 0x01;  // open interrupts on p2_0
    PICTL |= 0x08;  // falling-edge
    IEN2  |= 0x02;  // open all group interrupts
    P2IFG &= 0xFE;  // reset the flag
...
{
	uint8 reg55val=0X00;//默认就是bright=2
  	switch(bright)
	{
		case 0://-2
			reg55val=0XB0;	 	 
...
{
	uint8 reg56val=0X40;//默认就是contrast=2
  	switch(contrast)
	{
		case 0://-2
			reg56val=0X30;	 	 
